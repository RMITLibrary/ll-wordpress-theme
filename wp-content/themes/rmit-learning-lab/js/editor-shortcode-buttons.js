if (typeof QTags !== 'undefined') {
    ...
    // QTags renders the toolbar itself and fires no event when it is done. As an
    // inline footer script this ran late enough not to care; enqueued, DOM ready can
    // beat the toolbar into existence, so wait for the first button to appear.
    jQuery(function ($) {
        var tries = 0;

        (function addSeparators() {
            var bar = $('#ed_toolbar'),
                first = bar.find('[id$="_ll_image"]');

            if (!first.length) {
                if (tries++ < 40) { setTimeout(addSeparators, 50); }
                return;
            }

            first.before('<span class="ll-sep" aria-hidden="true"></span>');
